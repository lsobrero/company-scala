...
class CompanyReportSpec extends AnyWordSpec with BeforeAndAfterAll with Matchers with ScalaFutures with OptionValues {

  import Company._

  private val server = ServiceTest.startServer(ServiceTest.defaultSetup.withJdbc(true)) { ctx =>
    new LagomApplication(ctx) with CompanyComponents with TestTopicComponents {
